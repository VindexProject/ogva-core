...
def download_binary(tag, args) -> int:
    ...
    if tag < "v20" and platform in ["x86_64-apple-darwin", "aarch64-apple-darwin"]:
        platform = "osx64"
    tarball = 'ogvacore-{tag}-{platform}.tar.gz'.format(
        tag=tag[1:], platform=platform)
    tarballUrl = 'https://github.com/ogvanetwork/ogva-core/{bin_path}/{tarball}'.format(
    ...
    ret = subprocess.run(['tar', '-zxf', tarball, '-C', tag,
                          '--strip-components=1',
                          'ogvacore-{tag}'.format(tag=filename, platform=args.platform)]).returncode
    if ret != 0:
...
def build_release(tag, args) -> int:
    githubUrl = "https://github.com/ogvanetwork/ogva-core"
    if args.remove_dir:
        if Path(tag).is_dir():
            shutil.rmtree(tag)
