...
The Ogva Core daemon can be configured to act as a trusted "border
router", implementing the ogva wire protocol and relay, making
...
The ZeroMQ feature in Ogva Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ogvad:
...
For instance:
    ...
    $ ogvad -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/ogvad.tx.raw \
...
These options can also be provided in ogva.conf.
...
From the perspective of ogvad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ogvad directly. Furthermore, no information is
...
using. Ogvad appends an up-counting sequence number to each
