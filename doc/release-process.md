...
* [ ] Update manpages, see [gen-manpages.sh](https://github.com/ogvanetwork/ogva-core/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
### Ogva Core maintainers/release engineers, suggestion for writing release notes
...
Checkout the Ogva Core version you'd like to build:
...
_Note: this step can be skipped if [our CI](https://github.com/ogvanetwork/ogva-core/blob/master/ci/test/00_setup_env.sh#L64) still uses bitcoin's SDK package (see SDK_URL)_
...
Add other builders keys to your gpg keyring, and/or refresh keys: See `../ogva/contrib/builder-keys/README.md`.
...
* Transfer `ogvacore-osx-unsigned.tar.gz` to macOS for signing
* Extract and sign:

    ```sh
    tar xf ogvacore-osx-unsigned.tar.gz
...
* Extract and sign:
    ...
    ```sh
    tar xf ogvacore-win-unsigned.tar.gz
...
- Detached signatures will then be committed to the [ogva-detached-sigs](https://github.com/ogvanetwork/ogva-core-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
* [ ] Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to GitHub as GitHub draft release.
    1. The contents of each `./ogva/guix-build-${VERSION}/output/${HOST}/` directory, except for
       `*-debug*` files.
       ...
       as save storage space *do not upload these to the ogva.org server*.

       ```sh
       find guix-build-${VERSION}/output/ -maxdepth 2 -type f -not -name "SHA256SUMS.part" -and -not -name "*debug*" -exec scp {} user@ogva.org:/var/www/bin/ogva-core-${VERSION} \;
       ```
...
* [ ] Update the ogva.org download links
* [ ] Ensure that docker hub images are up to date

### Announce the release:
* [ ] Release on Ogva forum: https://www.ogva.org/forum/topic/official-announcements.54/ (necessary so we have a permalink to use on twitter, reddit, etc.)
...
* [ ] Update Core and User docs (docs.ogva.org)
* [ ] Test Docker build runs without error in Ogvamate
...
```sh
xcrun notarytool submit ogvacore-{version}-{x86_64, arm64}-apple-darwin.dmg --keychain-profile "AC_PASSWORD" --wait
```
...
After successfully notarizing the .dmg file, extract `Ogva-Qt.app` from the .dmg.
To verify that the notarization process was successful, run the following command:

```sh
spctl -a -vv -t install Ogva-Qt.app
```

Replace `Ogva-Qt.app` with the path to your .app file. This command checks whether your .app file passes Gatekeeper’s
